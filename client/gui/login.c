...
//click Dang nhap
int check=0;
static void on_button_signin_clicked(GtkWidget *widget,gpointer window)
{
	gchar *str;
	...
	// on_button_signin_clicked_pass();

}
// void on_button_signin_clicked_pass(GtkWidget *widget,gpointer window)
// {
// 	if(strcmp(gtk_entry_get_text(GTK_ENTRY(window)),"1")==0&&check==1)
// 	main_play();
// 	else show_error();
// }
...
void login(int argc,char *argv[])
{
	...
	gtk_fixed_put(GTK_FIXED(fixed),entry_tk,180,80);
	gtk_widget_set_size_request(entry_tk,100,30);
	entry_pass=gtk_entry_new();
	gtk_fixed_put(GTK_FIXED(fixed),entry_pass,180,120);
	gtk_widget_set_size_request(entry_pass,100,30);
	...
	g_signal_connect(G_OBJECT(button_signin),"clicked",G_CALLBACK(on_button_signin_clicked),entry_tk);
	//g_signal_connect(G_OBJECT(button_signin),"clicked",G_CALLBACK(on_button_signin_clicked_pass),entry_pass);
